...
impl TmuxHarness {
    ...
    pub fn cleanup(&mut self) {
        ...
        cleanup_test_managed_tmux_session(&self.session_name);
    }

    /// Buddy-managed tmux session name derived from this harness session.
    pub fn managed_session_name(&self) -> String {
        test_managed_tmux_session_name(&self.session_name)
    }
}
...
/// Return true when a tmux session currently exists.
pub fn tmux_session_exists(session: &str) -> HarnessResult<bool> {
    let output = Command::new("tmux")
        .arg("has-session")
        .arg("-t")
        .arg(session)
        .output()
        .map_err(|e| format!("failed checking tmux session `{session}`: {e}"))?;
    Ok(output.status.success())
}

/// Remove the buddy-managed session generated by UI harness test config.
fn cleanup_test_managed_tmux_session(requested_session: &str) {
    let managed = test_managed_tmux_session_name(requested_session);
    let _ = run_tmux(["kill-session", "-t", &managed]);
}

/// Compute the managed session name buddy generates in UI regression tests.
fn test_managed_tmux_session_name(requested_session: &str) -> String {
    let owner_prefix = "buddy-ui-test-agent";
    let raw = requested_session.trim();
    if raw.starts_with(owner_prefix) {
        raw.to_string()
    } else {
        format!("{owner_prefix}-{}", sanitize_tmux_fragment(raw, "session"))
    }
}

/// Match buddy session-name sanitization for deterministic cleanup.
fn sanitize_tmux_fragment(raw: &str, fallback: &str) -> String {
