...
pub(crate) struct ShellToolResult {
    pub exit_code: i32,
    pub stdout: String,
    pub stderr: String,
}
...
pub(crate) fn parse_shell_tool_result(result: &str) -> Option<ShellToolResult> {
...
pub(crate) fn tool_result_display_text(result: &str) -> String {
    if let Some(payload) = parse_tool_result_payload(result) {
        if let Some(text) = payload.as_str() {
...
pub(crate) fn parse_tool_arg(args: &str, key: &str) -> Option<String> {
...
pub(crate) fn quote_preview(text: &str, max_len: usize) -> String {
